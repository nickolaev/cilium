...
func TestDesiredStateHonorsBackendGates(t *testing.T) {
	...
	state, err := desiredState([]*loadbalancer.Frontend{mixedFamilyFrontend}, nil, nil, false, false)
	require.NoError(t, err)
	require.Empty(t, state.Services)
	require.Empty(t, state.Policies)

	_, err = desiredState([]*loadbalancer.Frontend{mixedFamilyFrontend}, nil, nil, true, false)
	require.Error(t, err)
}
