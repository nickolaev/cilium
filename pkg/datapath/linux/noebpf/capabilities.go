...
func CurrentCapabilities() []Capability {
	return []Capability{
		...
		{Name: "CiliumNetworkPolicy / CiliumClusterwideNetworkPolicy", Level: SupportLevelSupportedSubset, Notes: "L3/L4 allow and deny rules, selector matching, namespace labels, named ports, endPort, ipBlock/except, CIDR groups, service targets, and ICMP; deny rules render before allow rules on overlap"},
		{Name: "Cilium CIDR groups", Level: SupportLevelSupportedSubset, Notes: "CIDRGroupRef, CIDRGroupSelector, and external-group ToGroups/FromGroups expand to resolved CIDRs"},
		{Name: "Cilium service targets", Level: SupportLevelSupportedSubset, Notes: "K8sService and K8sServiceSelector ToServices entries expand to matching service backends"},
		{Name: "ICMP policy", Level: SupportLevelSupportedSubset, Notes: "ICMPv4 and ICMPv6 type rules are rendered directly"},
		{Name: "Cilium deny policies", Level: SupportLevelSupportedSubset, Notes: "L3/L4 ingressDeny and egressDeny rules"},
		{Name: "Proxy-dependent authentication", Level: SupportLevelUnsupported, Notes: "authentication policy requires proxy-layer enforcement"},
		{Name: "Identity-aware selectors and special entities", Level: SupportLevelUnsupported, Notes: "entity selectors and identity-aware semantics are unavailable"},
		{Name: "Node selectors", Level: SupportLevelUnsupported, Notes: "node-level policy targets are not supported"},
		{Name: "External groups / CIDR-group integrations", Level: SupportLevelUnsupported, Notes: "external group and CIDR-group selectors are unavailable"},
