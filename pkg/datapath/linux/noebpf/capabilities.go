...
func CurrentCapabilities() []Capability {
	return []Capability{
		...
		{Name: "CiliumNetworkPolicy / CiliumClusterwideNetworkPolicy", Level: SupportLevelSupportedSubset, Notes: "L3/L4 allow and deny rules, selector matching, namespace labels, named ports, endPort, and ipBlock/except; deny rules render before allow rules on overlap"},
		{Name: "Cilium deny policies", Level: SupportLevelSupportedSubset, Notes: "L3/L4 ingressDeny and egressDeny rules"},
		{Name: "ICMP policy", Level: SupportLevelUnsupported, Notes: "ICMP rule semantics are not implemented"},
		{Name: "Service-based policy targets", Level: SupportLevelUnsupported, Notes: "toServices targets are not supported"},
