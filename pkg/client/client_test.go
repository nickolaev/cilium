	...
	testCases := []struct {
		...
		},
		{
				...
				KubeProxyReplacement: &models.KubeProxyReplacement{
					Features: &models.KubeProxyReplacementFeatures{
						Annotations: []string{
							...
							"No-eBPF CiliumNetworkPolicy / CiliumClusterwideNetworkPolicy: supported-subset (L3/L4 allow and deny rules, selector matching, namespace labels, named ports, endPort, and ipBlock/except; deny rules render before allow rules on overlap)",
							"No-eBPF Cilium deny policies: supported-subset (L3/L4 ingressDeny and egressDeny rules)",
							"No-eBPF ICMP policy: unsupported (ICMP rule semantics are not implemented)",
							"No-eBPF Service-based policy targets: unsupported (toServices targets are not supported)",
			...
			expectedContains: []string{
				...
				"ICMP policy: unsupported (ICMP rule semantics are not implemented)",
			},
		},
	}
