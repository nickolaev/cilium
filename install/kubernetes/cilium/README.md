...
| bpf.datapathMode | string | `veth` | Mode for Pod devices for the core datapath (auto, veth, linux-route, netkit, netkit-l2). Note linux-route is an experimental no-BPF forwarding mode and netkit is incompatible with TPROXY (`bpf.tproxy`). |
